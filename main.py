...
def count_answers_per_fragebogen(frageboegen: Dict[str, pd.DataFrame]) -> Dict[str, int]:
...
def calculate_statistics(normalized_df: pd.DataFrame, original_df: pd.DataFrame) -> Dict:
    """
...
def print_statistics(stats: Dict, fragebogen_name: str):
    ...
    for col in norm_stats['mean'].index:
        print(f"{col:<15} {norm_stats['mean'][col]:>9.3f} {norm_stats['std'][col]:>9.3f} "
    ...
    print("-" * 80)
    for col in norm_stats['mean'].index:
        print(f"{col:<15} {norm_stats['q25'][col]:>9.3f} {norm_stats['q75'][col]:>9.3f} "
    ...
    for col in orig_stats['mean'].index:
        print(f"{col:<15} {orig_stats['mean'][col]:>9.3f} {orig_stats['std'][col]:>9.3f} "
              f"{orig_stats['median'][col]:>9.3f} {orig_stats['min'][col]:>9.3f} {orig_stats['max'][col]:>9.3f}")
    
    print(f"\n{'Frage':<15} {'Q25':<10} {'Q75':<10} {'Schiefe':<12} {'Kurtosis':<10}")
    print("-" * 80)
    for col in orig_stats['mean'].index:
        print(f"{col:<15} {orig_stats['q25'][col]:>9.3f} {orig_stats['q75'][col]:>9.3f} "
              f"{orig_stats['skewness'][col]:>11.3f} {orig_stats['kurtosis'][col]:>9.3f}")
    
    print("\n" + "="*80)


def visualize_specific_fragebogen(frageboegen: Dict[str, pd.DataFrame], fragebogen_name: str, normalize=True):
    """
    ...
    for i, col in enumerate(numeric_df_renamed.columns):
        axes[i].hist(numeric_df_renamed[col].dropna(), bins=20, edgecolor='black', color='steelblue')
    ...
    if normalize:
        ...
        axes[0].set_title(f"Boxplot (Original) – {fragebogen_name}", fontsize=14, fontweight='bold')
        ...
        axes[1].set_title(f"Boxplot (Z-Score normalisiert) – {fragebogen_name}", fontsize=14, fontweight='bold')
    ...
    if numeric_df.shape[1] > 1 and numeric_df.shape[1] <= 50:
        ...
        sns.heatmap(corr_renamed, annot=False, fmt='.2f', cmap='coolwarm', 
                    square=True, linewidths=0.5, cbar_kws={"shrink": 0.8})
        plt.title(f"Korrelationsmatrix – {fragebogen_name}", fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.show()
    elif numeric_df.shape[1] > 50:
        print(f"Zu viele Spalten ({numeric_df.shape[1]}) für detaillierte Korrelationsmatrix - übersprungen.")
    else:
    ...
    if numeric_df.shape[1] > 1 and numeric_df.shape[1] <= 50:
        ...
        g.fig.suptitle(f"ClusterMap (Hierarchisches Clustering) – {fragebogen_name}", 
                       fontsize=16, fontweight='bold', y=0.98)
        plt.show()
        
        print("✓ ClusterMap erstellt - Fragen sind nach Ähnlichkeit gruppiert!")
    elif numeric_df.shape[1] > 50:
        print(f"Zu viele Spalten ({numeric_df.shape[1]}) für ClusterMap - übersprungen.")
